...
namespace Masterplan.Tools.Generators
{
    internal class Book
    {
        private static readonly string[] FPrepositions =
            { "and", "in", "of", "with", "without", "against", "for", "to" };

        private static readonly string[] FAbout = { "about", "on", "concerning", "regarding" };
        ...
        public static string Title()
        {
            ...
            switch (Session.Random.Next(5))
            {
                ...
                {
                    // The NOUN's NOUN
                    var concrete1 = Session.Random.Next(2) == 0;
                    var concrete2 = Session.Random.Next(2) == 0;
                ...
                case 1:
                {
                    ...
                    var concrete1 = Session.Random.Next(2) == 0;
                    var concrete2 = Session.Random.Next(2) == 0;
                    title = noun_phrase(concrete1, concrete1) + " " + Preposition() + " " +
                            noun_phrase(concrete2, concrete2);
                }
                    break;
                case 2:
                {
                    // VERBING the NOUN
                    var concrete = Session.Random.Next(2) == 0;
            ...
            if (Session.Random.Next(10) == 0)
            {
                ...
                var volume = Session.Random.Next(5);
                switch (volume)
                {
                    case 0:
                        title += ", " + type + " one";
        ...
        private static string noun_phrase(bool concreteNoun, bool article)
        {
            var np = Noun(concreteNoun);

            var plural = false;
            if (concreteNoun && Session.Random.Next(5) == 0)
            {
                // Pluralise
                np += "s";
                plural = true;
        ...
        private static string Noun(bool concrete)
        {
            ...
            var index = Session.Random.Next(list.Count);
            return list[index];
        }
        ...
        {
            var index = Session.Random.Next(FPrepositions.Length);
            return FPrepositions[index];
        }

        private static string About()
        {
            var index = Session.Random.Next(FAbout.Length);
            return FAbout[index];
        }
    }
}
