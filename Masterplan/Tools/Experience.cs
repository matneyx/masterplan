...
namespace Masterplan.Tools
{
    internal static class Experience
    {
        ...
        public static int GetHeroLevel(int xp)
        {
            ...
            {
                var xpForLevel = GetHeroXp(level);
                var xpPlusOne = GetHeroXp(level + 1);

                if (xp >= xpForLevel && xp < xpPlusOne)
                    return level;
            }
        ...
        public static int GetCreatureLevel(int xp)
        {
            ...
            {
                var xpForLevel = GetCreatureXp(level);
                if (xp < xpForLevel)
                    return level - 1;
            }
        ...
        public static int GetChallengeLevel(int xp, Party p)
        {
            ...
            {
                var xpForLevel = GetCreatureXp(level) * p.Size;
                var diff = Math.Abs(xp - xpForLevel);
